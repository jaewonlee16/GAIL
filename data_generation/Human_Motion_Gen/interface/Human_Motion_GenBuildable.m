...
% 
classdef Human_Motion_GenBuildable < coder.ExternalDependency
    ...
    methods (Static)
        ...
        function updateBuildInfo(buildInfo, cfg)
            ...
            if(~strcmp(foldername, 'interface') || ~strcmp(solvername, 'Human_Motion_Gen'))
                solverpath = fullfile(buildablepath, 'Human_Motion_Gen');
            end
            solverInfo = struct();
            solverInfo.solvername = 'Human_Motion_Gen';
        ...
        function [output,exitflag,info] = forcesInitOutputsMatlab()
            ...
            outputs_x01 = coder.nullcopy(zeros(21, 1));
            outputs_x02 = coder.nullcopy(zeros(21, 1));
            outputs_x03 = coder.nullcopy(zeros(21, 1));
            outputs_x04 = coder.nullcopy(zeros(21, 1));
            outputs_x05 = coder.nullcopy(zeros(21, 1));
            outputs_x06 = coder.nullcopy(zeros(21, 1));
            outputs_x07 = coder.nullcopy(zeros(21, 1));
            outputs_x08 = coder.nullcopy(zeros(21, 1));
            outputs_x09 = coder.nullcopy(zeros(21, 1));
            outputs_x10 = coder.nullcopy(zeros(21, 1));
        ...
        function [output,exitflag,info] = forcesCallWithParams(params)
            [output,exitflag,info] = Human_Motion_GenBuildable.forcesCall(params.lb, params.ub, params.x0, params.xinit, params.all_parameters, params.num_of_threads);
        end

        function [output,exitflag,info] = forcesCall(lb, ub, x0, xinit, all_parameters, num_of_threads)
            solvername = 'Human_Motion_Gen';
            ...
            [output_c, exitflag_c, info_c] = Human_Motion_GenBuildable.forcesInitOutputsC(); %#ok<ASGLU>
            ...
            [output, exitflag, info] = Human_Motion_GenBuildable.forcesInitOutputsMatlab();
    ...
    methods (Static, Access = private)
        function [output,exitflag,info] = forcesInitOutputsC()
            ...
            outputs_x01 = coder.nullcopy(double(zeros(21, 1)));
            outputs_x02 = coder.nullcopy(double(zeros(21, 1)));
            outputs_x03 = coder.nullcopy(double(zeros(21, 1)));
            outputs_x04 = coder.nullcopy(double(zeros(21, 1)));
            outputs_x05 = coder.nullcopy(double(zeros(21, 1)));
            outputs_x06 = coder.nullcopy(double(zeros(21, 1)));
            outputs_x07 = coder.nullcopy(double(zeros(21, 1)));
            outputs_x08 = coder.nullcopy(double(zeros(21, 1)));
            outputs_x09 = coder.nullcopy(double(zeros(21, 1)));
            outputs_x10 = coder.nullcopy(double(zeros(21, 1)));
            output = struct('x01', outputs_x01,...
                            'x02', outputs_x02,...
                            'x03', outputs_x03,...
                            'x04', outputs_x04,...
