...
% 
% Human_Motion_Gen_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME)
%
% This function generates a Simulink Model with Fixed-step solver type named MODELNAME to add the Human_Motion_Gen FORCESPRO solver as 
% a MATLAB Function Simulink Block named BLOCKNAME so that it can be used for Simulink simulation and 
% Simulink Code Generation.
%
% Human_Motion_Gen_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK)
% can be used to generate a MATLAB Function Simulink Block with compact inputs and outputs.
%
% Human_Motion_Gen_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK, USEEXISTINGMODEL)
% can be used if the selected Simulink Model already exists in order to add the Simulink Block to it.
%
% Human_Motion_Gen_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK, USEEXISTINGMODEL, OPENSIMULINKMODEL)
% can be used to select whether to open the simulink model.
function Human_Motion_Gen_createCoderMatlabFunction(modelname, blockname, useCompactBlock, useExistingModel, openSimulinkModel)

    if nargin < 1 || isempty(modelname)
        modelname = 'Human_Motion_Gen_model';
    end
    if ~isvarname(modelname)
        error('Modelname must be a valid variable name');
    end
    if nargin < 2 || isempty(blockname)
        blockname = 'Human_Motion_Gen_block';
    end
    ...
    function_name = 'Human_Motion_Gen_coderFunction.m';
    position = [170, 99, 650, 350];
    parameter_sizes = struct('lb', '[195,1]',...
                             'ub', '[195,1]',...
                             'x0', '[210,1]',...
                             'xinit', '[15,1]',...
                             'all_parameters', '[230,1]',...
                             'num_of_threads', '[1,1]');
    if useCompactBlock
        function_name = 'Human_Motion_Gen_coderFunctioncompact.m';
