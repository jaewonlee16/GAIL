...
a Python wrapper for a fast solver generated by FORCESPRO v6.2.0
   ...
   OUTPUT = Human_Motion_Gen_py.Human_Motion_Gen_solve(PARAMS) solves a multistage problem
   subject to the parameters supplied in the following dictionary:
       PARAMS['lb'] - matrix of size [195x1]
       PARAMS['ub'] - matrix of size [195x1]
       PARAMS['x0'] - matrix of size [210x1]
       PARAMS['xinit'] - matrix of size [15x1]
       PARAMS['all_parameters'] - matrix of size [230x1]
       PARAMS['num_of_threads'] - scalar

   OUTPUT returns the values of the last iteration of the solver where
       OUTPUT['x01'] - column vector of length 21
       OUTPUT['x02'] - column vector of length 21
       OUTPUT['x03'] - column vector of length 21
       OUTPUT['x04'] - column vector of length 21
       OUTPUT['x05'] - column vector of length 21
       OUTPUT['x06'] - column vector of length 21
       OUTPUT['x07'] - column vector of length 21
       OUTPUT['x08'] - column vector of length 21
       OUTPUT['x09'] - column vector of length 21
       OUTPUT['x10'] - column vector of length 21

   [OUTPUT, EXITFLAG] = Human_Motion_Gen_py.Human_Motion_Gen_solve(PARAMS) returns additionally
   ...
   [OUTPUT, EXITFLAG, INFO] = Human_Motion_Gen_py.Human_Motion_Gen_solve(PARAMS) returns 
...
try:
    _lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'Human_Motion_Gen/lib/Human_Motion_Gen_withModel.so'))
    csolver = getattr(_lib,'Human_Motion_Gen_solve')
except:
    _lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'Human_Motion_Gen/lib/libHuman_Motion_Gen_withModel.so'))
    csolver = getattr(_lib,'Human_Motion_Gen_solve')

internal_mem = getattr(_lib,'Human_Motion_Gen_internal_mem')

class Human_Motion_Gen_params_ctypes(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
    _fields_ = [('lb', ctypes.c_double * 195),
('ub', ctypes.c_double * 195),
('x0', ctypes.c_double * 210),
('xinit', ctypes.c_double * 15),
('all_parameters', ctypes.c_double * 230),
('num_of_threads', ctypes.c_uint),
]

Human_Motion_Gen_params = {'lb' : np.array([]),
'ub' : np.array([]),
'x0' : np.array([]),
'xinit' : np.array([]),
...
Human_Motion_Gen_params_types = {'lb' : np.float64,
'ub' : np.float64,
'x0' : np.float64,
'xinit' : np.float64,
...
class Human_Motion_Gen_outputs_ctypes(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
...
Human_Motion_Gen_outputs = {'x01' : np.array([]),
'x02' : np.array([]),
'x03' : np.array([]),
'x04' : np.array([]),
'x05' : np.array([]),
...
class Human_Motion_Gen_info(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
...
class Human_Motion_Gen_mem(ctypes.Structure):
    pass
...
csolver.argtypes = ( ctypes.POINTER(Human_Motion_Gen_params_ctypes), ctypes.POINTER(Human_Motion_Gen_outputs_ctypes), ctypes.POINTER(Human_Motion_Gen_info), ctypes.POINTER(Human_Motion_Gen_mem), ctypes.POINTER(FILE))
csolver.restype = ctypes.c_int

internal_mem.argtypes = [ctypes.c_uint]
internal_mem.restype = ctypes.POINTER(Human_Motion_Gen_mem)

def Human_Motion_Gen_solve(params_arg):
    '''
a Python wrapper for a fast solver generated by FORCESPRO v6.2.0
 ...
 See also COPYING
    ...
    # convert parameters
    params_py = Human_Motion_Gen_params_ctypes()
    for par in params_arg:
        ...
        try:
            ...
            if isinstance(getattr(params_py, par), ctypes.Array):
                params_arg[par] = np.require(params_arg[par], dtype=Human_Motion_Gen_params_types[par], requirements='F')
    ...
    outputs_py = Human_Motion_Gen_outputs_ctypes()
    info_py = Human_Motion_Gen_info()
    if sys.version_info.major == 2:
        ...
        try:
            PyFile_AsFile.restype = ctypes.POINTER(FILE)
            exitflag = _lib.Human_Motion_Gen_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), internal_mem(0), PyFile_AsFile(fp) , _lib.Human_Motion_Gen_adtool2forces )
        ...
        try:
            if sys.platform.startswith('win'):
                exitflag = _lib.Human_Motion_Gen_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), internal_mem(0), None , _lib.Human_Motion_Gen_adtool2forces)
            else:
                exitflag = _lib.Human_Motion_Gen_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), internal_mem(0), fp , _lib.Human_Motion_Gen_adtool2forces)
            libc.fclose(fp)
    ...
    # convert outputs
    for out in Human_Motion_Gen_outputs:
        Human_Motion_Gen_outputs[out] = npct.as_array(getattr(outputs_py,out))

    return Human_Motion_Gen_outputs,int(exitflag),info_py

solve = Human_Motion_Gen_solve
